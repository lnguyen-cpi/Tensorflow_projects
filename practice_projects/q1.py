...
square_dig = tf.diag(tf.range(1, 7, 1))
print(sess.run(square_dig))
...
square = tf.random_normal([10, 10], mean=0, stddev=-1)
det = tf.matrix_determinant(square)
print(sess.run(det))
...
def mean_square(): return tf.reduce_mean(tf.square(x - y))
def sum_absolute(): return tf.reduce_sum(tf.abs(x - y))

result = tf.cond(tf.less(
